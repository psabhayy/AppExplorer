...
def parse_explore_rsp(rsp):
    try:
        if "candidates" not in rsp or not rsp["candidates"]:
            raise KeyError("'candidates' key not found or empty in the response")
        ...
        print_with_color(json.dumps(rsp, indent=2), "red")
        return ["ERROR"]
